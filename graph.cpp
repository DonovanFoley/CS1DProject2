...
int Graph::findMSTTotalWeight(const GraphStructure& graph, int startVertex) {
    if (graph.find(startVertex) == graph.end()) {
        std::cerr << "Start vertex " << startVertex << " is not present in the graph." << std::endl;
        return 0; // Early exit if the start vertex is not in the graph.
    }
    ...
    // Initialize keys and MST set for all vertices present in the graph
    for (const auto& item : graph) {
        key[item.first] = std::numeric_limits<int>::max();
        inMST[item.first] = false;
    }
    ...
    while (!pq.empty()) {
        ...
        if (inMST[u]) continue; // Skip this vertex if it's already included in MST
        inMST[u] = true; // Mark this vertex as included in MST
        totalWeight += key[u]; // Add the key value of u to the total weight of MST

        if (graph.find(u) == graph.end()) continue; // Skip if no edges are present

        // Get all edges safely using extractEdges
        for (const Edge& edge : extractEdges(graph, u)) {
...
std::vector<Edge> Graph::extractEdges(const GraphStructure& graph, int vertex) {
    std::vector<Edge> edges;
    if (graph.find(vertex) != graph.end()) {
        std::priority_queue<Edge, std::vector<Edge>, std::greater<Edge>> temp = graph.at(vertex);
        while (!temp.empty()) {
